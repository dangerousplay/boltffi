...
- `clang` available (used to compile the JNI bridge), or a configured `jni_compiler` override
...
For Linux targets, you can specify a minimum glibc version to produce binaries compatible with older distributions. This requires `cargo-zigbuild`:
...
[targets.java.jvm]
enabled = true
host_targets = ["darwin-arm64", "linux-x86_64.2.17"]
```

The `.2.17` suffix is forwarded to `cargo zigbuild` as `--target x86_64-unknown-linux-gnu.2.17`. See [Custom Build Commands](#custom-build-commands) for more details.
...
BoltFFI uses `cargo build` by default. For cross-compilation or specific toolchain requirements, you can override this with `cargo-zigbuild`, `cross`, or any cargo-compatible tool.

### Using cargo-zigbuild for Linux glibc Targeting

When packaging for Linux servers, you often need to target a specific minimum glibc version. `cargo-zigbuild` uses Zig as a linker to achieve this:
...
BoltFFI passes `--target x86_64-unknown-linux-gnu.2.17` to `cargo zigbuild`, producing a binary compatible with glibc 2.17+.

You can combine glibc-targeted Linux with other platforms:
...
### Fully Zig-based Cross-Compilation

For a pure Zig toolchain (no extra cross-linker required), combine `cargo zigbuild` with `zig cc` as the JNI compiler:
...
[targets.java.jvm]
enabled = true
host_targets = ["linux-x86_64.2.17", "linux-aarch64.2.17"]
jni_compiler = "zig cc"
```

BoltFFI invokes the JNI compiler as `zig cc -target x86_64-linux-gnu.2.17` (or `aarch64-linux-gnu.2.17` for the second target). The glibc version and zig triple are derived automatically from `host_targets`.

This is the recommended setup when cross-compiling Linux JNI libraries from macOS — no Docker, no cross-linker wrapper, just `cargo install cargo-zigbuild` and a Zig installation.

### Using cross for Docker-based Builds

`cross` runs builds inside Docker containers with pre-configured toolchains:
...
Override the build command for a single invocation without changing your config:

```bash
boltffi pack java --release --cargo-build-cmd zigbuild
boltffi build apple --cargo-build-cmd cross
```

The `--cargo-build-cmd` flag takes precedence over `[cargo].build_command` in `boltffi.toml`.

See [Configuration](/docs/configuration#cargo-build-command) for the full reference.
