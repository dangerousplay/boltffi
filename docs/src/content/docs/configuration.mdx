...
By default, BoltFFI uses `cargo build` to compile your Rust library. You can override this with alternative build tools like [`cargo-zigbuild`](https://github.com/rust-cross/cargo-zigbuild) or [`cross`](https://github.com/cross-rs/cross) for cross-compilation.

### Configuration

Set the build command in `boltffi.toml`:
...
BoltFFI resolves shorthand values into the appropriate program and subcommand:

| Config value       | Resolved command     |
|--------------------|----------------------|
...
Any `"program subcommand"` pair is accepted, so you can plug in other cargo-compatible build tools.

### CLI Override

The `--cargo-build-cmd` flag overrides the config file value for a single invocation:

```bash
boltffi pack java --cargo-build-cmd zigbuild
boltffi build android --cargo-build-cmd cross
boltffi pack all --release --cargo-build-cmd "cargo zigbuild"
```

### Using cargo-zigbuild

[`cargo-zigbuild`](https://github.com/rust-cross/cargo-zigbuild) uses Zig as a linker to cross-compile Rust code for Linux with a specific minimum glibc version. This is useful when you need to produce binaries compatible with older Linux distributions.
...
[targets.java.jvm]
enabled = true
host_targets = ["linux-x86_64.2.17"]
```

The `.2.17` suffix on `linux-x86_64` tells `cargo zigbuild` to target glibc 2.17 (CentOS 7 / RHEL 7 era). This suffix is forwarded verbatim to the `--target` argument:

```
cargo zigbuild --target x86_64-unknown-linux-gnu.2.17
```

The glibc version suffix is only valid on Linux targets (`linux-x86_64`, `linux-aarch64`). Specifying it on macOS or Windows targets is an error.

Common glibc version targets:

| glibc | Compatible with |
|-------|-----------------|
| `2.17` | CentOS/RHEL 7+, Debian 8+, Ubuntu 14.04+ |
| `2.28` | CentOS/RHEL 8+, Debian 10+, Ubuntu 18.10+ |
| `2.31` | Debian 11+, Ubuntu 20.04+ |
| `2.35` | Ubuntu 22.04+ |

Without a glibc suffix, `cargo zigbuild` targets the system default glibc version.

### Using cross

[`cross`](https://github.com/cross-rs/cross) provides Docker-based cross-compilation environments. It mirrors the `cargo` CLI but runs builds inside pre-configured containers with the right toolchains.
...
Since `cross` is a standalone binary (not a cargo subcommand), BoltFFI invokes it directly as `cross build` and does not forward toolchain selectors like `+nightly`.

### Combining with Other Cargo Settings

The build command works alongside other `[cargo]` settings:
...
Extra cargo arguments from `[cargo.command_args]`, `[cargo.global_args]`, and the `--cargo-arg` CLI flag are appended after the subcommand, so the full invocation becomes:

```
cargo zigbuild --target <triple> --features server
```
...
Linux targets in `host_targets` accept an optional glibc version suffix for use with `cargo zigbuild`:
...
[targets.java.jvm]
enabled = true
host_targets = ["darwin-arm64", "linux-x86_64.2.17", "linux-aarch64.2.28"]
```

The glibc suffix (e.g. `.2.17`) is forwarded to the `--target` argument of the build command. Artifacts are written to the standard directory without the suffix.
...
By default, BoltFFI discovers an appropriate C compiler to link the JNI glue (`clang` on Darwin/Linux, `clang-cl` or `cl` on Windows). Override this with the `jni_compiler` setting:
...
`zig cc` is the natural companion to `cargo zigbuild` for fully Zig-based Linux cross-compilation from macOS. Specify `"zig"` or `"zig cc"` and BoltFFI derives the correct `-target` argument automatically from `host_targets`:
...
BoltFFI invokes zig as:

```
zig cc -target x86_64-linux-gnu.2.17
```

The zig triple is derived from the Rust triple by dropping the vendor component and mapping `darwin` → `macos`. The glibc version suffix is appended automatically when the target uses the GNU ABI (e.g. `-gnu`).

When using `zig cc` for cross-compilation, no separate cross-linker installation (such as `zig-cc` wrappers or `CARGO_TARGET_*_LINKER` env vars) is required alongside `cargo zigbuild`.

#### Using clang with an explicit target

Clang-compatible compiler names (e.g. `clang`, `clang-18`) get `--target {rust_triple}` added automatically:

```toml
[targets.java.jvm]
jni_compiler = "clang-18"
```

#### Using a custom compiler

Any other executable is used as-is with no additional target argument:

```toml
[targets.java.jvm]
jni_compiler = "/opt/toolchain/bin/arm-linux-gnueabihf-gcc"
```
...
[cargo]
build_command = "zigbuild"   # optional: use cargo-zigbuild for Linux glibc targeting
